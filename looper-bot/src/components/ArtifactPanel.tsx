...
function EmptyArtifact() {
  return (
    <div className="empty-artifact">
      <p>Ask Looper to show web results, charts, notes, records, code, images, or progress here.</p>
    </div>
  );
}

function renderArtifact(artifact: LooperArtifact, mermaidState: MermaidState) {
      ...
      <div className="mermaid-stack">
        ...
        {mermaidState.error ? (
          <details className="mermaid-repair">
            <summary>Looper repaired this chart so it would still display.</summary>
    ...
    <section className="thumbnail-board">
      <header className="thumbnail-board-meta">
        <div>
          ...
          <p>{(board.references || []).length} Bill reference image{(board.references || []).length === 1 ? "" : "s"} loaded</p>
        </div>
      ...
      {images.length > 0 ? (
        ...
        <div className="thumbnail-empty">
          <p>Bill reference image loaded. Ask Looper: “Generate a 16:9 thumbnail of me about Cursor agents.”</p>
        </div>
      )}
    </section>
  );
